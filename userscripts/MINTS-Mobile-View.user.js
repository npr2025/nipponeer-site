...
// @match        https://www.mints.courts.go.jp/user/*
// @match        https://mints.courts.go.jp/user/*
// @run-at       document-end
// @grant        none
// @noframes
...
(() => {
    ...
    style.textContent = `
      ...
      *, *::before, *::after {
      ...
      @media (max-width: 700px) {
        ...
        table.${TABLE_CLASS} td:last-child {
          border-bottom: 0 !important;
        }

        table.${TABLE_CLASS} td::before {
          content: attr(data-mints-label);
          display: block !important;
        ...
        table.${TABLE_CLASS} td[data-mints-label=""]::before {
          display: none !important;
        }

        table.${TABLE_CLASS} td a,
        table.${TABLE_CLASS} td button,
        table.${TABLE_CLASS} td .btn {
          min-height: 44px !important;
  ...
  function getHeaders(table) {
    if (table.tHead?.rows?.length) {
      const lastHeadRow = table.tHead.rows[table.tHead.rows.length - 1];
      return Array.from(lastHeadRow.cells || []);
    }
  ...
  function processTable(table) {
    ...
    if (table.dataset.mintsMobileDone === '1') return;
